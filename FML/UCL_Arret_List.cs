...
{
    public partial class UCL_Arret_List : UserControl
    {
       private static UCL_Arret_List UserArret;
        private Db_EleveBookEntities4 db;
        LR_ArretTable RS = new LR_ArretTable();
        public static int IDselect;

        public UCL_Arret_List()
        {
            InitializeComponent();
        ...
        public static UCL_Arret_List Instance
        {
            get
            {
                if (UserArret == null)
                {
                    UserArret = new UCL_Arret_List();
                }
                return UserArret;
            }
        }

        private void UCL_Arret_List_Load(object sender, EventArgs e)
        {
        ...
        public void ActualiserDataGridView()
        {
            ...
            foreach (var l in db.LR_ArretTable)
            {
        ...
        {
            FRML_ArretAjoutetModifier frmArret = new FRML_ArretAjoutetModifier(this);
        ...
        private void btnModifier_Click(object sender, EventArgs e)
        {
            ...
            {
                FRML_ArretAjoutetModifier frmRF = new FRML_ArretAjoutetModifier(this);
                ...
                for (int i = 0; i < dgvArret.Rows.Count; i++)
                {
                    if ((bool)dgvArret.Rows[i].Cells[0].Value == true) // si ligne selectionnée
                    {
                        ...
                        RS = db.LR_ArretTable.SingleOrDefault(s => s.Id == IDselect);
                        if (RS != null)
                        {
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i < dgvArret.Rows.Count; i++) //nombre de ligne selectionnées
                    {
                        ...
                        {
                            CL.CLSL_ArretTable cLS_Arret = new CL.CLSL_ArretTable();
        ...
        {
            db = new Db_EleveBookEntities4();
            var listeRecherche = db.LR_ArretTable.ToList();  //liste de recherche == liste des eleves
            ...
            try
            {
                using (SaveFileDialog SFD = new SaveFileDialog() { Filter = "Excel Workbook | *.xlsx", ValidateNames = true })
                {
                    if (SFD.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        var lisetRE = db.LR_ArretTable.ToList();
                        int i = 2;
        ...
        private void btnRapportTOUT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                db = new Db_EleveBookEntities4();
                var listREEleves = db.LR_ArretTable.ToList();
                frmRE.RPafficher.LocalReport.ReportEmbeddedResource = "Eleve_Book.Rapport.rptl_arret_tout.rdlc";
        ...
        {
            dgvArret.Sort(dgvArret.Columns[7], ListSortDirection.Ascending);
        }
