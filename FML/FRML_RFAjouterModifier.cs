...
{
    public partial class FRML_RFAjouterModifier : Form
    {
        private UserControl usEleve;  //pr le remplacer par USER_List_Client
        CL.CLSL_R_famillialesTable classRF = new CL.CLSL_R_famillialesTable();
        UCL_RS_List userRS = new UCL_RS_List();
        private Db_EleveBookEntities4 db;

        public FRML_RFAjouterModifier(UserControl userC)
        {
            InitializeComponent();
            this.usEleve = userC;
        }

        private void FRML_RFAjouterModifier_Load(object sender, EventArgs e)
        {
        ...
        private void btnEnregistrer_Click_1(object sender, EventArgs e)
        {
            ...
            else if (lblTitre.Text == "AJOUTER  DONNEES  FAMILIALES") //tous les champs remplis
            {
                ...
                if (classRF.Ajouter_RF(cmbNom.Text, cmbPrenom.Text, txtPrenomPere.Text, txtPrenomMere.Text, txtNomMere.Text, cmbNbreFreres.Text, cmbNbreSoeurs.Text, txtFonctionPere.Text, txtFonctionMere.Text, txtAdresseParents.Text, txtTelephoneParents.Text, txtPersonneAContacter.Text, txtAutres.Text) == true)
                {
                    ...
                    (usEleve as FM.UCL_RF_List).ActualiserDataGridViewRF();
            ...
            else  //si lblTitre  == modifier eleve
            {
                ...
                {
                    classRF.Modifier_RF(UCL_RF_List.IDselectRF, cmbNom.Text, cmbPrenom.Text, txtPrenomPere.Text, txtPrenomMere.Text, txtNomMere.Text, cmbNbreFreres.Text, cmbNbreSoeurs.Text, txtFonctionPere.Text, txtFonctionMere.Text, txtAdresseParents.Text, txtTelephoneParents.Text, txtPersonneAContacter.Text, txtAutres.Text);
                    MessageBox.Show("Données modifiés avec succès", "MODIFICATION", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);

                    (usEleve as UCL_RF_List).ActualiserDataGridViewRF();  //actualiser dataGridView
                      Close();                  
                }
                else
                {
