...
{
    public partial class FRM_ArretAjoutetModifier2 : Form
    {
        private UserControl usEleve;  //pr le remplacer par USER_List_Client
        CL.CLS_ArretTable classArret = new CL.CLS_ArretTable();
        UC_Arret_List userRArret = new UC_Arret_List();
        private Db_EleveBookEntities4 db;

        public FRM_ArretAjoutetModifier2(UserControl user)
        {
            InitializeComponent();
            this.usEleve = user;
        }

        private void FRM_ArretAjoutetModifier2_Load(object sender, EventArgs e)
        {
        ...
        private void btnEnregistry_Click(object sender, EventArgs e)
        {
            ...
            else if (lblTitre.Text == "AJOUTER UNE PUNITION") //tous les champs remplis
            {
                ...
                if (classArret.Ajouter_RArret(cmbNom.Text, cmbPrenom.Text, cmbPunition.Text, txtMotif.Text, txtDate.Text, txtAutorite.Text, txtAutres.Text) == true)
                {
                    ...
                    if (d == DialogResult.OK)
                        this.Close();
                    (usEleve as FM.UC_Arret_List).ActualiserDataGridView();
            ...
            else  //si lblTitre  == modifier eleve
            {
                ...
                {
                    classArret.Modifier_RArret(UC_Arret_List.IDselect, cmbNom.Text, cmbPrenom.Text, cmbPunition.Text, txtMotif.Text, txtDate.Text, txtAutorite.Text, txtAutres.Text);
                    MessageBox.Show("Données modifiés avec succès", "MODIFICATION", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);

                    (usEleve as UC_Arret_List).ActualiserDataGridView();  //actualiser dataGridView
                    this.Close();
                }
                else
                {
