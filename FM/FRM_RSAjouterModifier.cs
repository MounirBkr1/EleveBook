...
{
    public partial class FRM_RSAjouterModifier : Form
    {
        private UserControl usEleve;  //pr le remplacer par USER_List_Client
        CL.CLS_R_SecondaireTable ClassRS = new CL.CLS_R_SecondaireTable();
        UC_RS_List userRS = new UC_RS_List();
        private Db_EleveBookEntities4 db;

        public FRM_RSAjouterModifier(UserControl userC)
        {
            InitializeComponent();
            this.usEleve = userC;
        }

        private void FRM_RSAjouterModifier_Load(object sender, EventArgs e)
        {
            ...
            if (lblTitreRS.Text == "AJOUTER  RENSEIGNEMENTS  SECONDAIRES")
            {
                ...
                this.cmbPrenom.DataSource = null;
        ...
        private void btnEnregistrer_Click(object sender, EventArgs e)
        {
            ...
            else if (lblTitreRS.Text == "AJOUTER  RENSEIGNEMENTS  SECONDAIRES") //tous les champs remplis
            {
                ...
                if (ClassRS.Ajouter_RS(cmbNom.Text, cmbPrenom.Text, cmbRegion.Text, cmbVille.Text, txtDateNaissance.Text, txtLieuNaissance.Text, cmbGSanguin.Text, txtSomme.Text, txtCIN.Text, txtCCP.Text, txtCM.Text, txtAutres.Text) == true)
                {
                    ...
                    (usEleve as FM.UC_RS_List).ActualiserDataGridViewRS();
            ...
            else  //si lblTitre  == modifier eleve
            {
                ...
                {
                    ClassRS.Modifier_RS(UC_RS_List.IDselectRS, cmbNom.Text, cmbPrenom.Text, cmbRegion.Text, cmbVille.Text, txtDateNaissance.Text, txtLieuNaissance.Text, cmbGSanguin.Text, txtSomme.Text, txtCIN.Text, txtCCP.Text, txtCM.Text, txtAutres.Text);
                    MessageBox.Show("Elève modifié avec succès", "MODIFICATION", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);

                    (usEleve as UC_RS_List).ActualiserDataGridViewRS();  //actualiser dataGridView
                    Close();
        ...
        //remplir le combobox Province a partir de database et en prenant compte de ce qui est choisi sur combobox Rregion
