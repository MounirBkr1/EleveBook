...
{
    public partial class FRM_PTC_AjouterModifier : Form
    {
        private UserControl usEleve;  //pr le remplacer par USER_List_Client
        CL.CLS_PtcTable classPTC = new CL.CLS_PtcTable();
        UC_PTC_List userRArret = new UC_PTC_List();
        private Db_EleveBookEntities4 db;

        public FRM_PTC_AjouterModifier(UserControl user)
        {
            InitializeComponent();
            this.usEleve = user;
        }

        private void FRM_PTC_AjouterModifier_Load(object sender, EventArgs e)
        {
        ...
        private void btnEnregistry_Click(object sender, EventArgs e)
        {
            ...
            else if (lblTitre.Text == "AJOUTER   PTC") //tous les champs remplis
            {
                ...
                if (classPTC.Ajouter_PTC(cmbNom.Text, cmbPrenom.Text, cmbPTCExemption.Text, cmbNbreJours.Text, txtMotifPTC.Text, txtDate.Text, txtAutres.Text) == true)
                {
                    ...
                    if (d == DialogResult.OK)
                        this.Close();
                    (usEleve as FM.UC_PTC_List).ActualiserDataGridView();
            ...
            else  //si lblTitre  == modifier eleve
            {
                ...
                {
                    classPTC.Modifier_PTC(UC_PTC_List.IDselect, cmbNom.Text, cmbPrenom.Text, cmbPTCExemption.Text, cmbNbreJours.Text, txtMotifPTC.Text, txtDate.Text, txtAutres.Text);
                    MessageBox.Show("Données modifiés avec succès", "MODIFICATION", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);

                    (usEleve as UC_PTC_List).ActualiserDataGridView();  //actualiser dataGridView
                    this.Close();
                }
                else
                {
