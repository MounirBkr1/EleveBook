...
{
    public partial class FRM_ActiviteAjouterModifier : Form
    {
      
        private UserControl usEleve;  //pr le remplacer par USER_List_Client
        CL.CLS_ActivitesTable classRA = new CL.CLS_ActivitesTable();
        Uc_Activite_List userAct = new Uc_Activite_List();
        public int IDEleve;

       
        public FRM_ActiviteAjouterModifier(UserControl userC)
        {
            InitializeComponent();
            this.usEleve = userC;
        ...
        private void FRM_ActiviteAjouterModifier_Load(object sender, EventArgs e)
        {
        ...
        private void btnEnregistry_Click(object sender, EventArgs e)
        {
            ...
            else if (lblTitre.Text == "AJOUTER UNE ACTIVITE") //tous les champs remplis
            {
                ...
                if (classRA.Ajouter_Eleve(cmbNom.Text, cmbPrenom.Text, para, aguerrissement, corveteEte, semaineEleve, gantBlanc, permisConduire, najeur, fumeur, txtAutres.Text) == true)
                {
                    ...
                    (usEleve as FM.Uc_Activite_List).ActualiserDataGridViewAct();
            ...
            else  //si lblTitre  == modifier eleve
            {
                ...
                {
                    classRA.Modifier_Eleve(Uc_Activite_List.IDselect, cmbNom.Text, cmbPrenom.Text,  para, aguerrissement, corveteEte, semaineEleve, gantBlanc, permisConduire, najeur, fumeur, txtAutres.Text);
                    MessageBox.Show("Elève modifié avec succes", "MODIFICATION", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);

                    (usEleve as FM.Uc_Activite_List).ActualiserDataGridViewAct();  //actualiser dataGridView
                    Close();
                }
                else
                {
